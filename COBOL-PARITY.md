...
| intrinsic functions | 110 | **110 (100%)** | 82 (75%) |
...
**Boundary intrinsics (28 not wired).** Two kinds, distinguished so the gap is not read as latent work:
...
| `MODULE-DATE` | no fixed oracle: a compile-time stamp (cobc embeds the build moment; the interpreter has no compile step) |
| `MODULE-FORMATTED-DATE` | no fixed oracle: a compile-time stamp (cobc embeds the build moment; the interpreter has no compile step) |
...
| `MODULE-TIME` | no fixed oracle: a compile-time stamp (cobc embeds the build moment; the interpreter has no compile step) |
...
| `WHEN-COMPILED` | no fixed oracle: a compile-time stamp (cobc embeds the build moment; the interpreter has no compile step) |
