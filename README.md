...
**`gnucobol-rs` is a Rust COBOL runtime and interpreter that is continuously differential-tested against stable and current GnuCOBOL. The project maintains a content-addressed, licence-checked valid-COBOL corpus of 6442 admitted units — 4732 programs extracted step-by-step from the GnuCOBOL native Autotest suite, 512 NIST CCVS85 units, the official manual examples, GnuCOBOL-shipped programs, the Open Mainframe Project course, and 1095 real-world X-COBOL programs — every unit classified under an explicit validity profile (oracle, dialect, source format, compiler options, copybooks, runtime configuration), executed through both the pinned GnuCOBOL 3.2 oracle and the native-Rust candidate, and reported with byte-level accuracy and per-phase first-failure attribution. A frozen held-out repository partition measures generalization, and scalable correctness-gated workloads measure parsing, checking, prepared execution and arithmetic throughput.**

Under the hood it is a faithful 1:1 port of the entire GnuCOBOL 3.2 `libcob` runtime — all 13 admitted `.c` files — into safe Rust, proven byte-identical to a pinned, locally-built GnuCOBOL 3.2 oracle, plus an independently written interpreter (`cobrun` — the author's from-scratch claim; strict clean-room provenance is not independently verifiable from the committed record, see [methodology](docs/methodology/parser-front-end-provenance.md)) that parses and *executes* real COBOL programs on that runtime. No C is linked. It is not a library with tests; it is a *compatibility court* where "correct" never means "our reading of a spec" — it means byte-for-byte identical to the admitted cobc (GnuCOBOL) 3.2.0, where every claim is mechanically chained to a replayable receipt, and where every boundary is stated as loudly as every capability. The current corpus evidence: **1910 valid executable programs**, **3222 valid compile-only programs**, **70 valid module programs**, and **196 byte-identical observable matches** in the testsuite differential lane.**
...
The admitted NIST CCVS85 v4.0 corpus — 512 units (459 COBOL + 51 CLBRY + 2 DATA) — is materialized and executed through both the pinned GnuCOBOL 3.2 oracle and the native-Rust `cobrun` front-end in an isolated Docker environment (GNURUST.CCVS85.2/.3/.4). The oracle compiles 370 of 391 executable candidates and runs 304 of them; `cobrun` accepts and executes 87 and fails closed on the rest, with **27 byte-identical raw-output matches**, 42 output divergences, and 16 generated-file divergences — every unit explicitly classified (0 timeouts), none silently dropped, and the two-run determinism verified in fresh containers. One-command replay: `bash lab/ccvs85/run-docker.sh` · full ledger: [`reports/ccvs85/summary.md`](reports/ccvs85/summary.md).

The replay uses a dedicated rootless Docker daemon whose mutable state is stored beneath `$GNURUST_CCVS85_DOCKER_ROOT`. The location is configurable and should point to a filesystem with substantial free space. The harness verifies that it is not connected to the production Docker socket and that all daemon data, build cache, temporary files, and outputs remain beneath the configured root.

```sh
export GNURUST_CCVS85_DOCKER_ROOT=/path/on/a-large-filesystem/gnucobol-rs
export GNURUST_CCVS85_BASE_IMAGE=/path/to/ubuntu-minimal-image.tar
...
> **As of gnucobol-rs 0.8.57 (2026-06):** 13/13 `libcob` files ported 1:1 · 110/110 intrinsics in the runtime · 176 sealed courts.
...
```sh
bash lab/oracle/cobol_frontend_sweep.sh     # 193-program front-end byte sweep, gate FAIL=0
bash lab/valid-corpus/run-docker.sh        # 6442-unit corpus pipeline (extract+replay+candidate+unify+gate)
```

**Reviewer entry points** — [`STATUS.md`](STATUS.md) (live current-state authority) · [`COBOL-PARITY.md`](COBOL-PARITY.md) / [`FILE-PARITY.md`](FILE-PARITY.md) (live language + file coverage) · [`GAP-ANALYSIS.md`](GAP-ANALYSIS.md) (105/105 catalogued gaps fixed, 0 open) · [`reports/negative-capabilities.json`](reports/negative-capabilities.json) (non-claims) · [`reports/casefiles/`](reports/casefiles/) (176 forensic case files).
...
| `GNURUST.FRONTEND.1` | the independently written front-end: parse + **execute** a subset to `cobc`-identical stdout (provenance-qualified in the methodology docs) |

Of the **176** sealed courts, **140** are `GNURUST.*` (the open LGPL runtime + front-end layer); **31** are `KOBOLD.*`; the remainder are framework courts. The machine-readable form is [`reports/claim-ladder.json`](reports/claim-ladder.json), and each court's full forensic record (`casefile.json` + SARIF 2.1.0 + in-toto v1 + DSSE envelope) lives under [`reports/casefiles/`](reports/casefiles/). 

The **full 176-court ledger** is in [`docs/sealed-courts.md`](docs/sealed-courts.md).
...
**Legal skim, in one line:** the runtime is a **faithful LGPL-3.0-or-later derivative** of `libcob` — ported statement-by-statement with upstream line citations (e.g. `// move.c:477`), so it is **not clean-room** and its license was **not freely chosen**; the `cobrun` front-end is *independently written* (the author's from-scratch claim; strict clean-room provenance is not independently verifiable from the committed record) but ships inside the LGPL crate; the `cobc`-driving tooling is **GPL-3.0-or-later**.

## How the runtime and parser were produced

The two halves of this project have **different derivation models**, and the wording matters:

- **Runtime** (`crates/gnucobol-rs`): a **faithful Rust port/derivative** of the admitted GnuCOBOL 3.2
  `libcob` sources — read directly and mirrored statement-by-statement with upstream line citations
  (`// move.c:477`), inheriting **LGPL-3.0-or-later**. It is **not** clean-room, and its license was not
  freely chosen. Full method: [`docs/methodology/libcob-rust-port.md`](docs/methodology/libcob-rust-port.md)
  + machine record [`reports/methodology/libcob-port-provenance.json`](reports/methodology/libcob-port-provenance.json).
- **Parser/front-end** (`src/frontend.rs` + `cobrun`): an **independently written** COBOL lexer/parser/
  executor, developed against the built oracle (behavioral differential), per the author's committed
  from-scratch claim. Strict "clean-room" process separation **cannot be independently verified** from
  the committed record (the tooling/consulted-materials history is UNKNOWN). Full audit:
  [`docs/methodology/parser-front-end-provenance.md`](docs/methodology/parser-front-end-provenance.md)
  + machine record [`reports/methodology/parser-provenance.json`](reports/methodology/parser-provenance.json).
