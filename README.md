...
| Axis | Meaning | Status |
|------|---------|--------|
| **byte-layout** | a field's bytes match GnuCOBOL's exactly | **first sealed slice (decimal)** |
| **runtime** | a runtime operation (`MOVE`, …) matches `libcob` | **first sealed slice (decimal MOVE)** |
...
reports, diagnostics — and **no lower layer is allowed to imply a higher layer**. Today only the
two lowest courts (storage bytes, `MOVE` bytes) are sealed. The full taxonomy is in
