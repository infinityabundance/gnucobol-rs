...
fn registry() -> Vec<RegistryRow<'static>> {
    vec![
        (
            "CCVS85",
            "NIST CCVS85 COBOL-85 Validation Suite (VERSION 4.0, 1992)",
            "historical conformance suite",
            "GNURUST.CCVS85.1",
            "required",
            "NIST CCVS85 newcob.val.Z (committed spine; mirrors e.g. github.com/.../nistcobol85)",
            "US Government work / public domain (NIST)",
            "lab/corpus/ccvs85/newcob.val.Z",
            "ccvs85",
        ),
        (
            "GNUCOBOL-TESTS",
            "GnuCOBOL 3.2 upstream test suite (tests/, incl. cobol85 NIST-derived)",
            "upstream compiler regression suite",
            "GNURUST.GNUCOBOL-TESTS.1",
            "high",
            "GnuCOBOL 3.2 admitted source tarball (research/gnucobol-3.2.tar.lz)",
            "GPL-3.0 / LGPL-3.0 (GnuCOBOL project)",
            "lab/admit/gnucobol-3.2/tests",
            "tree",
        ),
        (
            "GCOBOL",
            "GCC gcobol testsuite (cobol.dg, COBOL-2023 front end)",
            "upstream compiler regression suite",
            "GNURUST.GCOBOL.1",
            "medium-high",
            ...
            "lab/corpus/gcobol/gcc",
            "git",
        ),
        (
            "OPEN-CBS",
            "OpenCBS — Open-Source COBOL Defects Benchmark Suite (43 programs)",
            "independent defect corpus",
            "GNURUST.OPEN-CBS.1",
            "medium-high",
            "git https://github.com/PhaseChangeSoftware/cobol-defects-suite",
            "see repo LICENSE (Phase Change Software et al.)",
            "lab/corpus/opencbs/repo",
            "git",
        ),
        (
            "X-COBOL",
            "X-COBOL — Dataset of Open-Source COBOL Repositories (84 repos, 1255 files)",
            "independent real-world corpus",
            "GNURUST.XCOBOL.1",
            "medium",
            "Zenodo doi:10.5281/zenodo.14269462 (full record archive)",
            "per-repository upstream licenses (mined open-source)",
            "research",
            "zip:14269462.zip",
        ),
    ]
...
fn derive_one(
    root: &Path,
    e: &(&str, &str, &str, &str, &str, &str, &str, &str, &str),
) -> CorpusEntry {
...
fn derive(root: &Path) -> Atlas {
    Atlas {
        ...
        conformance_claim:
            "NONE — corpus custody/index across three evidence classes; compile/run/behaviour \
    ...
    for c in &a.corpora {
        rows.push_str(&format!(
            ...
            c.counts
                ...
                .collect::<Vec<_>>()
                .join(", "),
            // Intentionally an always-empty stub: the joined counts above already carry "cobol=N".
            "",
            c.claim,
        ));
    }
...
pub fn check(root: &Path) -> i32 {
    let committed: Atlas = match std::fs::read_to_string(root.join(RECEIPT_JSON))
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
    {
        ...
        None => {
            eprintln!(
                "GNURUST.COBOL-CORPUS-ATLAS.1 STALE: receipt missing (run `corpus-atlas generate`)"
            );
            return 1;
        }
    };
    ...
    for (c, f) in committed.corpora.iter().zip(&fresh.corpora) {
        ...
        if c.id != f.id
            || c.gate != f.gate
            || c.source != f.source
            || c.evidence_class != f.evidence_class
        {
            eprintln!(
                "GNURUST.COBOL-CORPUS-ATLAS.1 STALE: metadata drift for {}",
                c.id
            );
            return 1;
        }
        ...
        if f.status == "LOCAL" && (c.custody_id != f.custody_id || c.counts != f.counts) {
            eprintln!(
                "GNURUST.COBOL-CORPUS-ATLAS.1 STALE: custody drift for {} (re-run generate)",
                c.id
            );
            return 1;
        }
    ...
    let local = committed
        ...
        .count();
    println!(
        "GNURUST.COBOL-CORPUS-ATLAS.1: atlas STABLE ({} corpora, {} with committed local custody)",
        committed.corpora.len(),
        local
    );
    0
}
