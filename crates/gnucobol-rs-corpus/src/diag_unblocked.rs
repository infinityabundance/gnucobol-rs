...
/// The decision for one AT_CHECK: what to ignore, and why. (The decision is computed inline in
/// `decide_check`; this struct is documentation of the decision shape.)
fn decide_check(
    check: &SpannedCheck,
    source_revision: &str,
    source_file: &str,
...
/// Stable (sorted-key) JSON string used for manifest hashing and reproducibility compares.
...
pub fn cmd_transform(
    ...
    let _ = regenerate_patch_bytes(suite_src_root, source_revision)?;
    write_transform_artifacts(suite_src_root, report_root, &res)?;
    Ok(res)
}

/// Deterministically regenerate the `diagnostic-ignore.patch` bytes (no file writes) so the
/// reconcile command can prove the committed patch is reproducible from the pristine sources.
pub fn cmd_transform_bytes(
    suite_src_root: &Path,
    source_revision: &str,
) -> Result<Vec<u8>, String> {
    regenerate_patch_bytes(suite_src_root, source_revision)
}

/// The transformed `.at` file bytes (name -> bytes) for the given pristine suite source root.
pub fn transformed_tree(suite_src_root: &Path) -> Result<BTreeMap<String, Vec<u8>>, String> {
    ...
    for f in suite_at_files(suite_src_root) {
        ...
        let name = f
            ...
            .unwrap_or("")
            .to_string();
        let (trans, _) = transform_file(&text, &name, "stable-3.2");
        let edits = edits_for_file(&text, &trans).map_err(|e| format!("{name}: {e}"))?;
        out.insert(name, apply_edits(&text, edits).into_bytes());
    }
    Ok(out)
}

/// Rebuild the patch bytes from the pristine sources (shared by `cmd_transform` and
/// `cmd_transform_bytes`).
fn regenerate_patch_bytes(suite_src_root: &Path, source_revision: &str) -> Result<Vec<u8>, String> {
    ...
    for f in suite_at_files(suite_src_root) {
        ...
        let new_text = apply_edits(&text, edits);
        if text != new_text {
            patch.push_str(&unified_diff(&text, &new_text, &name));
        }
    }
    Ok(patch.into_bytes())
}

/// Write the transform artifacts (transformed tree, patch, manifests, csv, md) for a report
/// root. Extracted from `cmd_transform` so both the CLI and the reconcile path share it.
fn write_transform_artifacts(
    suite_src_root: &Path,
    report_root: &Path,
    res: &TransformResult,
) -> Result<(), String> {
    ...
    for f in suite_at_files(suite_src_root) {
        ...
        let text = String::from_utf8_lossy(&bytes).into_owned();
        let name = f.file_name().and_then(|n| n.to_str()).unwrap_or("");
        let (trans, _) = transform_file(&text, name, &res.source_revision);
    ...
    patch.push_str(&format!(
        "# source revision: {} — only proven compiler-diagnostic expected streams\n",
        res.source_revision
    ));
    patch
        .push_str("# become Autotest `ignore`; commands, exit statuses, source, runtime output,\n");
    ...
    md.push_str(&format!(
        "_transformer {TRANSFORMER_VERSION} · source revision `{}` · ",
        res.source_revision
    ));
    md.push_str(&format!(
...
"##;
    ...
    #[test]
    fn gate_rejects_status_zero_to_ignore() {
        // expected status 0 -> ignore (turning a must-pass check into a don't-care) must fail
        let src = "AT_SETUP([t])\nAT_CHECK([$COMPILE_ONLY prog.cob], [0], [], [])\nAT_CLEANUP\n";
        let (td, _) = scratch_gate_inputs(src, |s| {
            s.replace(
                "AT_CHECK([$COMPILE_ONLY prog.cob], [0], [], [])",
                "AT_CHECK([$COMPILE_ONLY prog.cob], [ignore], [], [])",
            )
        });
        let transformed = src.replace(
            "AT_CHECK([$COMPILE_ONLY prog.cob], [0], [], [])",
    ...
    #[test]
    fn gate_rejects_xfail_insertion() {
        // inserting AT_XFAIL_IF to excuse a group must fail
        let src = "AT_SETUP([t])\nAT_CHECK([$COMPILE_ONLY prog.cob], [1], [], [prog.cob:9: error: bad\n])\nAT_CLEANUP\n";
        let (td, _) = scratch_gate_inputs(src, |s| {
            s.replace("AT_SETUP([t])\n", "AT_SETUP([t])\nAT_XFAIL_IF([true])\n")
        });
        ...
        assert!(
            !v.failures.is_empty(),
            "gate must reject AT_XFAIL_IF insertion: {:?}",
            v.failures
        );
    }

    #[test]
    fn gate_rejects_generated_file_removal() {
        // removing a generated-file expectation (5th AT_CHECK argument) must fail
        let src =
            "AT_SETUP([t])\nAT_CHECK([$COMPILE prog.cob], [0], [], [], [out.txt])\nAT_CLEANUP\n";
        let (td, _) = scratch_gate_inputs(src, |s| {
            s.replace(
                "AT_CHECK([$COMPILE prog.cob], [0], [], [], [out.txt])",
                "AT_CHECK([$COMPILE prog.cob], [0], [], [])",
        ...
        assert!(
            !v.failures.is_empty(),
            "gate must reject generated-file expectation removal: {:?}",
            v.failures
        );
    }

    #[test]
    fn gate_rejects_dialect_flag_change() {
        // changing a dialect/source-format flag inside the command must fail
        let src = "AT_SETUP([t])\nAT_CHECK([$COMPILE_ONLY -std=cobol85 prog.cob], [1], [], [prog.cob:9: error: bad\n])\nAT_CLEANUP\n";
        let (td, _) = scratch_gate_inputs(src, |s| s.replace("-std=cobol85", "-std=cobol2002"));
        let transformed = src.replace("-std=cobol85", "-std=cobol2002");
        let patch_path = td.path().join("p.patch");
        std::fs::write(&patch_path, unified_diff(src, &transformed, "syn_move.at")).unwrap();
        let v = cmd_gate(
            &patch_path,
    ...
    #[test]
    fn gate_rejects_at_check_deletion() {
        // deleting an AT_CHECK entirely must fail (count + hunk-level proof)
        let src = "AT_SETUP([t])\nAT_CHECK([$COMPILE_ONLY prog.cob], [1], [], [prog.cob:9: error: bad\n])\nAT_CHECK([$COMPILE_ONLY prog2.cob], [1], [], [prog2.cob:9: error: bad\n])\nAT_CLEANUP\n";
        let (td, _) = scratch_gate_inputs(src, |s| {
            s.replace(
                "AT_CHECK([$COMPILE_ONLY prog2.cob], [1], [], [prog2.cob:9: error: bad\n])\n",
                "",
        ...
        assert!(
            !v.failures.is_empty(),
            "gate must reject AT_CHECK deletion: {:?}",
            v.failures
        );
    }

    #[test]
    fn gate_rejects_hiding_crash_or_timeout() {
        // a crash (139) or timeout (137) is signalled by a nonzero status; permissively
        // rewriting it to 0 while ignoring streams hides the failure and must fail
        for status in ["137", "139"] {
            let src = format!(
                "AT_SETUP([t])\nAT_CHECK([$COMPILE_ONLY prog.cob], [{status}], [], [prog.cob:9: error: bad\n])\nAT_CLEANUP\n"
            );
            let (td, _) = scratch_gate_inputs(&src, |s| {
                s.replace(
                    &format!("AT_CHECK([$COMPILE_ONLY prog.cob], [{status}]"),
                    "AT_CHECK([$COMPILE_ONLY prog.cob], [0]",
                )
            });
            let transformed = src.replace(
    ...
    #[test]
    fn semantic_stdout_of_cli_surface_is_never_ignored_by_transformer() {
        // `$COBC --version` stdout is a CLI surface, not a compiler diagnostic stream; the
        // TRANSFORMER must preserve it (the gate then trusts only the manifest the transformer
        // produced, so the whole chain refuses semantic-output ignores)
        let src = "AT_SETUP([t])\nAT_CHECK([$COBC --version], [0], [cobc (GnuCOBOL) 3.2.0\n], [])\nAT_CLEANUP\n";
        let (trans, errs) = transform_file(src, "syn_move.at", "stable-3.2");
        assert!(errs.is_empty(), "{errs:?}");
        let t = trans
            .iter()
            .find(|t| t.step_index == 0)
            .expect("check record");
        assert!(
            !t.disposition.ignores_stdout(),
            "stdout of a CLI-surface check must be preserved, got {:?}",
            t.disposition
        );
    }
