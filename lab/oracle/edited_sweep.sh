#!/usr/bin/env bash
# Edited-picture decode sweep (GNURUST.16a): build ONE COBOL program that MOVEs each value into its
# edited field and DISPLAYs the field's bytes (bracketed); compile+run with the built cobc; then the
# Rust `edited_rows` mirror decodes those bytes back and checks the recovered value + size. PASS=n FAIL=n.
set -u
