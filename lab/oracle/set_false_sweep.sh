#!/usr/bin/env bash
# SET condition-name TO FALSE differential sweep (GNURUST.12B). For each gen_set case, build a program
...
GEN="$ROOT/target/release/examples/gen_set_false"
ROWS="$ROOT/target/release/examples/set_false_rows"
...
while IFS='|' read -r label decl size def fls; do
  [ -z "$label" ] && continue
  case "$label" in \#*) continue ;; esac
  vals=$(render_def "$def")
  fval=$(render_false "$fls")
  prog="$TMP/$label.cob"
  {
    ...
    echo "       01 P PIC $decl."
    echo "          88 C VALUE$vals WHEN SET TO FALSE IS $fval."
