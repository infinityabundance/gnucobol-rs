...
if [ -x "$PREFIX/bin/cobc" ] && [ -x "$ROOT/lab/oracle/decimal_harness" ]; then
  ...
  # COBOL-ARCHAEOLOGY: distilled, source-free COBOL findings atlas generated from
  # reports/archaeology/findings.json (own-words facts; no quotes/sources/links) -- must stay fresh.
  if ( cd "$ROOT" && cargo run -q -p xtask -- archaeology check ) >/tmp/_arch_check 2>&1; then
    note "COBOL-ARCHAEOLOGY: distilled source-free findings atlas fresh"
  else
