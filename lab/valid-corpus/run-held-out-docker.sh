#!/usr/bin/env bash
# run-docker.sh — the ONE-COMMAND replay for GNURUST.VALID-PROGRAMS.HELD-OUT.1.
#
#   bash lab/valid-corpus/run-held-out-docker.sh [--require-no-regression]
#
# Runs the held-out evaluation (pure measurement), the overfitting checks and the generalization
# report TWICE in two fresh containers (fresh run dirs), compares the stable summaries for
# determinism, sanitizes privacy, and runs the corpus gate. Reuses the isolated-daemon +
# base-image machinery from the valid-corpus lane (same project-scoped docker root).
set -euo pipefail
...
[ -n "$GNURUST_VALID_CORPUS_BASE_IMAGE" ] || fail "GNURUST_VALID_CORPUS_BASE_IMAGE is required"
...
elif [ "${GNURUST_VALID_CORPUS_NETWORKLESS:-0}" = "1" ]; then
  echo "networkless mode: basing the court image on the ccvs85 court image (apt step skipped)"
  CCVS85_ROOT="${GNURUST_CCVS85_DOCKER_ROOT:?GNURUST_CCVS85_DOCKER_ROOT is required when GNURUST_VALID_CORPUS_NETWORKLESS=1}"
...
# ---------------------------------------------------------------------------------------------
# 5. two fresh full runs (held-out pipeline)
# ---------------------------------------------------------------------------------------------
for pass in a b; do
  info "run $pass/2 (fresh container)"
  CONTAINER="held-out-$RUN_ID-$pass"
  ...
  docker run --name "$CONTAINER" --rm \
    ...
    -v "$ROOT/lab/docker/valid-corpus/held-out-run.sh:/usr/local/bin/held-out-run.sh:ro" \
    ...
    "$IMAGE_TAG" /usr/bin/stdbuf -oL -eL /usr/local/bin/held-out-run.sh 2>&1 | tee "$PROJECT_DOCKER_ROOT/logs/held-out-$pass.log"
  RC=${PIPESTATUS[0]}
  set -e
  [ "$RC" = "0" ] || fail "held-out run $pass failed (exit $RC)"
done
...
stable = ("crate_version", "git_commit", "oracle", "held_out_totals", "held_out_first_failure", "overfit_gate", "generalization_held_out_files")
...
DET_RC=$?
set -e
[ "$DET_RC" = "0" ] || fail "held-out determinism check failed"

cp "$PROJECT_DOCKER_ROOT/logs/preflight.json" "$PROJECT_DOCKER_ROOT/run-evidence/held-out-preflight.raw.json"
cp "$OUT_DIR/pass-a/summary.json" "$PROJECT_DOCKER_ROOT/run-evidence/held-out-a.raw.json"
cp "$OUT_DIR/pass-b/summary.json" "$PROJECT_DOCKER_ROOT/run-evidence/held-out-b.raw.json"

# copy the evidence back into the repo
cp "$OUT_DIR/pass-a/summary.json" "$ROOT/reports/valid-corpus/held-out-docker-summary.json" 2>/dev/null || true
if grep -RInE '/home/|/run/media/|/mnt/|/media/' "$ROOT/reports/valid-corpus/held-out-docker-summary.json" 2>/dev/null; then
  fail "PRIVACY GATE: a host path leaked into the committed held-out evidence"
fi
echo "privacy gate: committed held-out evidence carries only symbolic storage aliases"
...
info "DONE — GNURUST.VALID-PROGRAMS.HELD-OUT.1 evidence run complete"
echo "  run-id:      $RUN_ID"
echo "  summary:     $ROOT/reports/valid-corpus/held-out-docker-summary.json"
