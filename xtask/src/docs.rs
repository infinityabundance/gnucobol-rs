...
fn src(root: &str) -> std::path::PathBuf {
    Path::new(root).join("docs-src")
}
fn sha_b(b: &[u8]) -> String {
...
fn oracle_str(root: &str) -> String {
    let mut recs: Vec<std::path::PathBuf> =
        std::fs::read_dir(Path::new(root).join("reports/receipts"))
            .map(|rd| {
                rd.flatten()
                    .map(|e| e.path().join("receipt.json"))
                    .filter(|p| p.exists())
                    .collect()
            })
            .unwrap_or_default();
    recs.sort();
    for r in recs {
        let v = read_json(&r)["oracle"]["version"]
            .as_str()
            .map(String::from);
        if let Some(v) = v {
            if !v.is_empty() {
...
fn verdict(root: &str, cid: &str) -> String {
    let jf = Path::new(root)
        .join("reports/casefiles")
        .join(cid)
        .join("casefile.json");
    if !jf.exists() {
        return "❔ —".into();
    }
    let v = read_json(&jf)["results"]["verdict"]
        .as_str()
        .unwrap_or("")
        .to_string();
...
fn court_table(root: &str, courts: &[Value], title: &str, with_links: bool) -> String {
    let mut out = vec![
        format!("### {title}"),
        String::new(),
        "| 🔒 court | name | verdict | casefile |".to_string(),
        "|---|---|:---:|---|".to_string(),
    ];
    let mut sorted: Vec<&Value> = courts.iter().collect();
    sorted.sort_by(|a, b| {
        keyf(a["id"].as_str().unwrap_or("")).cmp(&keyf(b["id"].as_str().unwrap_or("")))
    });
    for c in sorted {
        let id = c["id"].as_str().unwrap_or("");
        let cf = if with_links {
            format!("[`reports/casefiles/{id}/`](reports/casefiles/{id}/)")
        } else {
            format!("`reports/casefiles/{id}/`")
        };
        out.push(format!(
            "| `{}` | {} | {} | {} |",
            id,
...
fn badge(label: &str, msg: &str, color: &str) -> String {
    ...
    format!(
        "![{label}](https://img.shields.io/badge/{}-{}-{color})",
        esc(label),
        esc(msg)
    )
}
...
fn machine_values(root: &str) -> BTreeMap<String, String> {
    ...
    let gnurust: Vec<Value> = courts
        .iter()
        .filter(|c| c["id"].as_str().unwrap_or("").starts_with("GNURUST."))
        .cloned()
        .collect();
    let kobold: Vec<Value> = courts
        .iter()
        .filter(|c| c["id"].as_str().unwrap_or("").starts_with("KOBOLD."))
        .cloned()
        .collect();
    let casefiles = std::fs::read_dir(Path::new(root).join("reports/casefiles"))
        .map(|rd| {
            rd.flatten()
                .filter(|e| e.path().join("casefile.json").exists())
                .count()
        })
        .unwrap_or(0);
    let neg_count = read_json(&Path::new(root).join("reports/negative-capabilities.json"))["count"]
        .as_i64()
        .unwrap_or(0);
    let sealed_byte = read_json(&Path::new(root).join("reports/kani-fuzz-coverage.json"))
        ["impl_courts"]
        .as_i64()
        .unwrap_or(0);
    let badges = [
        ...
        badge("sealed courts", &courts.len().to_string(), "brightgreen"),
    ...
    let dox_files = dox["files"]
        .as_array()
        .map(|a| a.len())
        .or_else(|| dox.as_array().map(|a| a.len()))
        .unwrap_or(0);
    ...
    // valid-COBOL corpus magnitudes (Phase 12 unified summary: same committed authority the
    // corpus gate reconciles, so the README never names a stale corpus figure).
    let corpus = read_json(&Path::new(root).join("reports/valid-corpus/summary.json"));
    let corpus_family = |f: &str| -> String {
        corpus["by_source_family"][f]
            .as_u64()
            .unwrap_or(0)
            .to_string()
    };
    let corpus_validity = |k: &str| -> String {
        corpus["by_validity_class"][k]
            .as_u64()
            .unwrap_or(0)
            .to_string()
    };
    m.insert(
        "corpus_total_units".into(),
        corpus["total_units"].as_u64().unwrap_or(0).to_string(),
    );
    m.insert(
        "corpus_testsuite_units".into(),
        corpus_family("gnucobol-testsuite"),
    );
    m.insert("corpus_ccvs85_units".into(), corpus_family("ccvs85"));
    m.insert("corpus_xcobol_units".into(), corpus_family("xcobol"));
    m.insert("corpus_omp_units".into(), corpus_family("omp"));
    m.insert("corpus_extras_units".into(), corpus_family("extras"));
    m.insert(
        "corpus_manual_units".into(),
        corpus_family("gnucobol-manual"),
    );
    m.insert(
        "corpus_valid_executable".into(),
        corpus_validity("VALID_EXECUTABLE_PROGRAM"),
    );
    m.insert(
        "corpus_valid_compile_only".into(),
        corpus_validity("VALID_COMPILE_ONLY_PROGRAM"),
    );
    m.insert(
        "corpus_valid_module".into(),
        corpus_validity("VALID_MODULE_PROGRAM"),
    );
    m.insert(
        "corpus_observable_matches".into(),
        // byte-identical observable matches live in the testsuite lane comparison ledger
        read_json(&Path::new(root).join("reports/gnucobol-testsuite/summary.json"))["summary"]
            ["comparison"]["observable_match"]
            .as_u64()
            .unwrap_or(0)
            .to_string(),
    );
...
fn header(model: &Value) -> String {
    ...
    let has_legacy = lp["legacy_paths"]
        .as_array()
        .map(|a| !a.is_empty())
        .unwrap_or(false);
    let (legacy_line, legacy_md) = if has_legacy {
        ...
        (
            format!("Legacy source preserved (lossless) under {p} (sha256 {s})."),
            format!(" Legacy source preserved losslessly under `{p}`."),
        )
    } else {
        (
            "No legacy source (born generated).".to_string(),
            " Born generated (no legacy source).".to_string(),
        )
    };
...
fn models(root: &str) -> Vec<Value> {
    let mut files: Vec<std::path::PathBuf> = std::fs::read_dir(src(root))
        .map(|rd| {
            rd.flatten()
                .map(|e| e.path())
                .filter(|p| {
...
pub fn run(cmd: &str, root: &str) -> i32 {
    ...
    match cmd {
        "generate" => {
            for m in &ms {
                let _ = std::fs::write(
                    Path::new(root).join(m["generated_path"].as_str().unwrap_or("")),
                    render(root, m),
                );
            }
            println!(
                "generated {} authoritative doc(s) from docs-src/ models",
                ms.len()
            );
            0
        }
        "check" => {
            ...
            let cl = read_json(&Path::new(root).join("reports/claim-ladder.json"));
            let cl_ids: Vec<String> = cl["courts"]
                .as_array()
                .map(|a| {
                    a.iter()
            ...
            for m in &ms {
                ...
                if !gp.exists() {
                    println!(
                        "NEG.DOC.LEGACY_NOT_REPLACED: {} not generated",
                        m["generated_path"].as_str().unwrap_or("")
                    );
                    bad += 1;
                    continue;
                }
                let actual = std::fs::read_to_string(&gp).unwrap_or_default();
                if actual != render(root, m) {
                    println!(
                        "NEG.DOC.MANUAL_EDIT: {} != regenerated (hand-edited or stale)",
                        m["generated_path"].as_str().unwrap_or("")
                    );
                    bad += 1;
                }
                let declares_version = m["machine_fields"]
                    .as_array()
                    .map(|a| a.iter().any(|x| x == "version"))
                    .unwrap_or(false);
                if declares_version && !actual.contains(&format!("gnucobol-rs {cv}")) {
                    println!(
                        "NEG.DOC.STALE_VERSION: {} does not name current version {cv}",
                        m["generated_path"].as_str().unwrap_or("")
                    );
                    bad += 1;
                }
                ...
                if lp.is_null() {
                    println!(
                        "NEG.DOC.INFORMATION_LOSS: {} model lacks legacy_preservation",
                        m["generated_path"].as_str().unwrap_or("")
                    );
                    bad += 1;
                } else if lp["legacy_paths"]
                    .as_array()
                    .map(|a| !a.is_empty())
                    .unwrap_or(false)
                {
                    ...
                    if !lpath.exists() {
                        println!(
                            "NEG.DOC.LEGACY_NOT_REPLACED: legacy source missing {}",
                            lp["legacy_paths"][0].as_str().unwrap_or("")
                        );
                        bad += 1;
                    } else if !lp["legacy_sha256"]
                        .as_array()
                        .map(|a| {
                            a.iter().any(|s| {
                                s.as_str()
                                    == Some(&sha_b(&std::fs::read(&lpath).unwrap_or_default()))
                            })
                        })
                        .unwrap_or(false)
                    {
                        println!(
                            "NEG.DOC.INFORMATION_LOSS: {} legacy_sha256 != actual",
                            m["generated_path"].as_str().unwrap_or("")
                        );
                        bad += 1;
                    }
                    if !lp["information_superset"].as_bool().unwrap_or(false)
                        || lp["missing_items"]
                            .as_array()
                            .map(|a| !a.is_empty())
                            .unwrap_or(false)
                    {
                        println!(
                            "NEG.DOC.INFORMATION_LOSS: {} not an information superset",
                            m["generated_path"].as_str().unwrap_or("")
                        );
                        bad += 1;
                    }
                ...
                if m["document_id"] == "STATUS" {
                    for cid in &cl_ids {
                        if !actual.contains(cid) {
                            println!(
                                "NEG.DOC.STALE_CLAIM: STATUS missing claim-ladder court {cid}"
                            );
                            bad += 1;
                        }
                    }
            ...
            if bad > 0 {
                println!("!! {bad} TRUST.4.DOCS finding(s)");
                return 1;
            }
        ...
        _ => {
            eprintln!("usage: docs generate|check");
            2
        }
    }
}
