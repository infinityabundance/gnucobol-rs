...
fn recdir(root: &str) -> PathBuf { Path::new(root).join("reports/receipts") }
fn manifest(root: &str) -> Value {
    serde_json::from_str(&std::fs::read_to_string(Path::new(root).join("lab/receipt/manifest.json")).unwrap_or_default()).unwrap_or(Value::Null)
}
...
fn oracle_version(root: &str) -> String {
    ...
    if !cobc.exists() { return "not-built".into(); }
    match std::process::Command::new(&cobc).arg("--version").env("LD_LIBRARY_PATH", prefix(root).join("lib")).output() {
        Ok(o) => String::from_utf8_lossy(&o.stdout).lines().next().unwrap_or("error").to_string(),
        Err(_) => "error".into(),
    }
}
...
fn dialect_ref(root: &str) -> Value {
    ...
    match std::fs::read_to_string(&dp).ok().and_then(|s| serde_json::from_str::<Value>(&s).ok()) {
        Some(p) => json!({"dialect_profile_id": p["profile_id"], "dialect_profile_sha256": p["profile_sha256"]}),
        None => json!({}),
    }
}
...
fn run_sweep(root: &str, script: &str, arg: Option<&str>) -> String {
    ...
    if !prefix(root).join("bin/cobc").exists() || !path.exists() { return "oracle-not-built".into(); }
    ...
    if let Ok(o) = cmd.output() {
        for line in String::from_utf8_lossy(&o.stdout).lines() {
            if line.starts_with("PASS=") && line.contains("FAIL=") { return line.trim().to_string(); }
        }
    }
    "no-result".into()
}
...
fn build(root: &str, code: &str, stamp: &str, git_commit: &str) -> Value {
    ...
    let replay = format!("bash lab/oracle/{}{}", m["sweep"].as_str().unwrap_or(""), m["arg"].as_str().map(|a| format!(" {a}")).unwrap_or_default());
    json!({
...
fn render_md(r: &Value) -> String {
    let nc = r["non_claims"].as_array().map(|a| a.iter().map(|n| format!("- {}", n.as_str().unwrap_or(""))).collect::<Vec<_>>().join("\n")).unwrap_or_default();
    let sup = r["superseded_by"].as_str().map(|s| format!(" (superseded_by {s})")).unwrap_or_default();
...
const NON_XTASK_COURTS: [&str; 3] = ["GNURUST.CCVS85.2", "GNURUST.CCVS85.3", "GNURUST.CCVS85.4"];

fn campaigns(root: &str) -> Vec<String> {
    manifest(root)["campaigns"].as_object().map(|m| {
        m.keys().cloned().filter(|k| !NON_XTASK_COURTS.contains(&k.as_str())).collect()
    }).unwrap_or_default()
}
    ...
    match cmd {
        "render-md" => {
            ...
            for code in campaigns(root) {
                ...
                if let Some(r) = std::fs::read_to_string(&jf).ok().and_then(|s| serde_json::from_str::<Value>(&s).ok()) {
                    let _ = std::fs::write(recdir(root).join(&code).join("receipt.md"), render_md(&r));
                    n += 1;
                }
            }
        ...
        "generate" => {
            for code in campaigns(root) {
                ...
                let _ = std::fs::write(d.join("receipt.json"), serde_json::to_vec_pretty(&r).unwrap_or_default());
                let _ = std::fs::write(d.join("receipt.md"), render_md(&r));
            }
            println!("generated {} receipts in reports/receipts/", campaigns(root).len());
            0
        }
        "check" => {
            let mut bad = 0;
            for code in campaigns(root) {
                ...
                let committed = match std::fs::read_to_string(&jf).ok().and_then(|s| serde_json::from_str::<Value>(&s).ok()) {
                    Some(c) => c,
                    None => { println!("DRIFT: {code} has no generated receipt.json"); bad += 1; continue; }
                };
                let fresh = build(root, &code, committed["generated_at"].as_str().unwrap_or(""), committed["git_commit"].as_str().unwrap_or(""));
                if evidence(&committed) != evidence(&fresh) { println!("DRIFT: {code} receipt evidence != live replay (regenerate)"); bad += 1; }
                if std::fs::read_to_string(d.join("receipt.md")).unwrap_or_default() != render_md(&committed) { println!("DRIFT: {code} receipt.md hand-edited"); bad += 1; }
            }
            // claim-ladder must only cite campaigns that have a generated receipt
            let cl: Value = serde_json::from_str(&std::fs::read_to_string(Path::new(root).join("reports/claim-ladder.json")).unwrap_or_default()).unwrap_or(Value::Null);
            let camps = campaigns(root);
            if let Some(courts) = cl["courts"].as_array() {
                for c in courts {
                    let cid = c["id"].as_str().unwrap_or("");
                    if cid.starts_with("GNURUST.") && camps.contains(&cid.to_string()) && !recdir(root).join(cid).join("receipt.json").exists() {
                        println!("DRIFT: claim-ladder cites {cid} with no generated receipt"); bad += 1;
                    }
                }
            }
            if bad > 0 { println!("!! {bad} receipt drift(s)"); return 1; }
            println!("receipts: all current, .md == render(.json), claim-ladder covered");
            0
        }
        _ => { eprintln!("usage: receipt generate|check|render-md"); 2 }
    }
}
