...
fn build_files() -> String {
    ...
    out.push_str(&format!(
        "## At a glance\n\n\
         **Built natively (ported / copied+wired / partial): {built} files** · \
         **active gaps: {active_gaps}** (cobc front-end + cob-config CLI) · \
         **partial (reproduced subset): {partial}** · \
         the rest is reference / test-corpus / Cargo-obviated. \
    ...
    out.push_str(&format!(
         ...
         **Active gaps: {active} files** -- the cobc compiler -> clean-room front-end (`cobrun`) and the \
         `cob-config` flags reporter; files reproduced as a verified subset are marked **PARTIAL** (with \
         a `proof` anchor), not GAP. Everything else is ported, copied, obviated by Cargo, or reference. \
         The plans are the road to full 1:1 parity.\n\n",
