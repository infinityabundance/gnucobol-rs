...
# GNURUST.COBC-RS.POLICY-COMPLETE.1 — wrapper option-policy registry completeness (every observed option has an explicit policy)

**Verdict: ORACLE-NOT-BUILT** · replay `oracle-not-built`

| field | value |
|-------|-------|
| campaign | `GNURUST.COBC-RS.POLICY-COMPLETE.1` |
| court | wrapper option-policy registry completeness (every observed option has an explicit policy) |
