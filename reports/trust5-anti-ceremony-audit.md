<!-- generated by xtask trust5 — do not edit by hand -->

# TRUST.5 — Anti-Ceremony Audit

> [!IMPORTANT]
> A court is **real** if corrupting/dropping/drifting/hand-editing its evidence can make a gate fail; **ceremonial** if it only restates that other evidence exists. This audit proves every court can fail.

- **A** hard (oracle/byte): 129  ·  **B** composed: 41  ·  **C** view: 6  ·  **D** staged: 0  ·  **F** ceremonial: **0**

## Classification

| court | class | can fail? | neg≥pos | new truth? |
|---|:---:|:---:|:---:|:---:|
| `DIALECT.PROFILE.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.10` | A | ✅ | ✅ | ✅ no |
| `GNURUST.11` | A | ✅ | ✅ | ✅ no |
| `GNURUST.12` | A | ✅ | ✅ | ✅ no |
| `GNURUST.12B` | A | ✅ | ✅ | ✅ no |
| `GNURUST.13` | B | ✅ | ✅ | ✅ no |
| `GNURUST.14` | A | ✅ | ✅ | ✅ no |
| `GNURUST.15` | A | ✅ | ✅ | ✅ no |
| `GNURUST.16` | A | ✅ | ✅ | ✅ no |
| `GNURUST.16C` | A | ✅ | ✅ | ✅ no |
| `GNURUST.17` | A | ✅ | ✅ | ✅ no |
| `GNURUST.18` | A | ✅ | ✅ | ✅ no |
| `GNURUST.19` | A | ✅ | ✅ | ✅ no |
| `GNURUST.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.3` | A | ✅ | ✅ | ✅ no |
| `GNURUST.4` | A | ✅ | ✅ | ✅ no |
| `GNURUST.5` | A | ✅ | ✅ | ✅ no |
| `GNURUST.6` | B | ✅ | ✅ | ✅ no |
| `GNURUST.7` | A | ✅ | ✅ | ✅ no |
| `GNURUST.8` | A | ✅ | ✅ | ✅ no |
| `GNURUST.9` | B | ✅ | ✅ | ✅ no |
| `GNURUST.ACCEPT.DISPLAY.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.ACCEPT.DISPLAY.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.BIGNUM.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.BUILD.PROFILE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CALL.EXTENSION.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CALL.LAYOUT.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CCVS85.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CCVS85.3` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CCVS85.4` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CLASS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COBC-RS.ARGS.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.COBC-RS.LAUNCHER.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.COBC-RS.NATIVE-MODE-BOUNDARY.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COBC-RS.PARALLEL.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.COBC-RS.POLICY-COMPLETE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COMMON.BOUNDCHECK.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COMMON.CBL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COMMON.NUMCHECK.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CORPUS.CUSTODY.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CORPUS.DEDUP.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.CORPUS.LICENCE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.COVERAGE.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.DECLARATIVES.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.DIALECT.RUNTIME.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.DIRECTIVE.VARIANCE.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.FILTER.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.FLOW.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.REWRITE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.SEQUENTIAL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.STATUS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILE.WRITE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.INDEXED.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.LINESEQ.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.LINESEQ.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.MAPPING.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.MULTI-RECORD-FD.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.OPEN.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.RELATIVE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.SEQ.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.SORT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.SORTENGINE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.SYS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FILEIO.VERB.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FLOAT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.FRONTEND.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-RUNTIME-MATH.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-RUNTIME-MATH.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.3` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.4` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.BOUNDARY-REDUCTION.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.GNUCOBOL-TESTSUITE.DIAGNOSTIC-UNBLOCKED.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.IF.EVALUATE.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.IF.NUMERIC.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INDEX.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INDEXED.FILE.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INITIALIZE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INSPECT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTPOW.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.CASE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.DATE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.INTEGER.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.LENGTH.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.MOD-REM.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.NUMVAL-C.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.NUMVAL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.INTRINSIC.ORD-CHAR.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.LINEAGE.CORPUS.20M.0` | A | ✅ | ✅ | ✅ no |
| `GNURUST.LINEAGE.CORPUS.20M.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.LINEAGE.CORPUS.20M.SMOKE` | A | ✅ | ✅ | ✅ no |
| `GNURUST.LOGICAL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.METHODOLOGY.LIBCOB.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.METHODOLOGY.PARSER.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.MLIO.GENERATE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MLIO.PARSE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MODULE.CALL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MODULE.CANCEL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MODULE.PARALLEL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MODULE.REGISTRY.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.MODULE.SEARCH.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.NUMCMP.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.ODO.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PERFORM.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PERFORMANCE.BUSINESS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PERFORMANCE.CORPUS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PERFORMANCE.FRONTEND.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PERFORMANCE.PREPARED.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PROCEDURE.FLOW.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.PUBLIC.CORPUS.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.PUBLIC.GAP.1` | B | ✅ | ✅ | ✅ no |
| `GNURUST.REFMOD.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.RELATIVE.FILE.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.REMAINDER.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.ROUND.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.ACCEPT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.ACCEPT.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.ATTR.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.COLOR.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.DISPLAY.2` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.DISPLAY.3` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.INIT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.LINEDIFF.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SCREENIO.NUMEDIT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SEARCH.TABLE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SIZE.ERROR.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SORT.MERGE.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.STRING.UNSTRING.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.SUBSCRIPT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.TABLE.PERFORM.SLICE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.ACCURACY.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.CCVS85.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.EXTRAS.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.GNUCOBOL-TESTSUITE.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.HELD-OUT.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.MANUAL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.OMP.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALID-PROGRAMS.XCOBOL.1` | A | ✅ | ✅ | ✅ no |
| `GNURUST.VALUE.NEGZERO.EDGE.1` | A | ✅ | ✅ | ✅ no |
| `KOBOLD.BANK.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.BANK.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.BANK.RECONCILE.1` | C | ✅ | ✅ | ✅ no |
| `KOBOLD.BENCH.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.CORPUS.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.CURRENCY.PROFILE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATA.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATA.3` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATA.4` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATA.5` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATA.6` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DATE.PROFILE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DB2HOST.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.DIFF.1` | C | ✅ | ✅ | ✅ no |
| `KOBOLD.ENTERPRISE.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.EXTRACT.PROFILE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.FILE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.LAYOUT.REDEFINES.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.OPERATOR.1` | C | ✅ | ✅ | ✅ no |
| `KOBOLD.PERF.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.PERF.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.PILOT-PACKET.1` | C | ✅ | ✅ | ✅ no |
| `KOBOLD.PILOT.RUN.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.PILOT.WORKFLOW.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.POSTING.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.PRIVACY.REDACTION.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.RECON.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.RECON.2` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.SCALE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.SENTINEL.PROFILE.1` | B | ✅ | ✅ | ✅ no |
| `KOBOLD.TOOLING.EXPORT.1` | C | ✅ | ✅ | ✅ no |
| `NIST-STYLE-FIXTURE-FORMAT.1` | B | ✅ | ✅ | ✅ no |
| `SIZE.ERROR.ATLAS.1` | A | ✅ | ✅ | ✅ no |
| `SUPPORT-PACKET.1` | C | ✅ | ✅ | ✅ no |
| `TRUST.5` | B | ✅ | ✅ | ✅ no |

## Staged (external proof pending)

- `LAMBDA.LIVE.1` — sibling kobold-lambda-layer; status awaiting_live_invocation (not green until a live AWS request id + matching output hash exist)

## Non-claims

- `NEG.TRUST5.AUDIT_NOT_CERTIFICATION`
- `NEG.TRUST5.CLASS_NOT_QUALITY_SCORE`
- `NEG.TRUST5.SNAPSHOT_NOT_LIVE`
- `NEG.TRUST5.NO_NEW_TRUTH`

