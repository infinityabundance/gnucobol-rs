...
`GNURUST.FILE.REWRITE.1` (sequential REWRITE in-place update, `rewrite_sweep` 1/0) — sealed 0.7.18; REWRITE overwrites ONLY the current record in place (same length), every other record untouched -- a surgical single-record update, not a file rewrite; LINE SEQ REWRITE/length-change/DELETE/indexed stay non-claims.\n\n`GNURUST.FILE.WRITE.1` (sequential WRITE byte effects`GNURUST.FILE.WRITE.1` (sequential WRITE byte effects, `write_seq_sweep` 2/0) — sealed 0.7.17; the configured line modes it left open are now sealed by `GNURUST.FILEIO.LINESEQ.1` (line-sequential WRITE config matrix, `lineseq_write_sweep` 8/0, sealed 0.7.42: COB_LS_FIXED/NULLS/VALIDATE byte-identical, validate rejects bytes < 0x20 with status 71). Its READ counterpart `GNURUST.FILEIO.LINESEQ.2` (line-sequential READ config matrix, `lineseq_read_sweep` 12/0, sealed 0.7.43) seals READ NEXT (
 fold, status 09/06/04/10, COB_LS_NULLS decode) and documents the IS_BAD_CHAR read-vs-write asymmetry. `GNURUST.FILEIO.SEQ.1` (RECORD SEQUENTIAL read/write, `seqrec_sweep` 9/0, sealed 0.7.44) seals fixed records + the variable-length COB_VARSEQ_FORMAT length prefix (0/1/2/3 = BE16+NUL/BE32/LE32/BE16).
